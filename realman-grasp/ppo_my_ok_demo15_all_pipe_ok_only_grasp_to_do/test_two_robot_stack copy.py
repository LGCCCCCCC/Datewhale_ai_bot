...
def test_set_pose_and_capture():
    ...
    try:
        ...
        if hasattr(agent_left, 'sensors') and camera_name_base in agent_left.sensors:
            ...
            if rgb_saved and depth_saved and intrinsics_cv is not None and cam2world_matrix is not None:
                ...
                if center_point_world is not None:
                    ...
                    try:
                        ...
                        urdf_path_for_ik = '/home/kewei/17robo/01mydemo/urdf_01/GEN72.urdf' # 使用简化版 URDF
            ...
            if final_rgb_exists and point_cloud_np is not None and intrinsics_exist:
                try:
                    graspnet_checkpoint = "/home/kewei/17robo/graspnet-baseline/checkpoint-rs.tar" # 您的检查点路径
                    predicted_grasps_cam_frame = get_grasp_poses_from_graspnet(
                        rgb_image_path=final_image_filename, # 使用最终姿态的 RGB
                        points_np=point_cloud_np,
                        intrinsics_matrix=intrinsics_cv_grasp,
                        checkpoint_path=graspnet_checkpoint
                    )
                    if predicted_grasps_cam_frame:
                         print(f"成功从 GraspNet 获取 {len(predicted_grasps_cam_frame)} 个抓取位姿 (相机坐标系)。")
                    else:
                 ...
                 if point_cloud_np is None: print("  Point cloud not generated.")
                 if not intrinsics_exist: print("  Intrinsics missing.")
            # --- GraspNet 调用结束 --- 
            ...
            if target_grasp_pose_base_sapien is not None: # 仅当成功获得目标 Sapien Pose 时执行
                print("\n" + "-"*15 + " 开始使用 GraspNet 位姿进行 IK 和移动 " + "-"*15)
                # 保存当前 QPos，以便 IK 失败时恢复（可选）
                # original_qpos_grasp_phase = agent_left.robot.get_qpos().clone() 

                # --- 准备 IK 输入 (使用当前的机器人状态) ---
                print("准备 IK 输入 (目标: GraspNet 位姿)...")
                urdf_path_for_ik = '/home/kewei/17robo/01mydemo/urdf_01/GEN72.urdf' # 保持不变
                ee_link_name = agent_left.ee_link_name # 保持不变
                current_sapien_qpos_grasp = agent_left.robot.get_qpos() # 获取 *当前* 的 qpos
                robot_articulation = agent_left.robot # 保持不变
                all_sapien_joint_names = [j.get_name() for j in robot_articulation.get_active_joints()] # 保持不变
                device = current_sapien_qpos_grasp.device
                dtype = current_sapien_qpos_grasp.dtype
                print(f"  目标 EE: '{ee_link_name}'")
                print(f"  目标位姿 (基座标系 sapien.Pose): p={target_grasp_pose_base_sapien.p}, q={target_grasp_pose_base_sapien.q}")
                
                # --- 调用 PyBullet IK (目标: GraspNet 位姿) ---
                ik_qpos_7d_grasp, ik_success_grasp = compute_ik_with_pybullet(
                    urdf_path=urdf_path_for_ik,
                    ee_link_name=ee_link_name,
                    target_pose_base=target_grasp_pose_base_sapien, # <<< 使用 GraspNet 的目标
                    current_sapien_qpos=current_sapien_qpos_grasp, # 使用当前 qpos
                    all_sapien_joint_names=all_sapien_joint_names,
                    device=device,
                    dtype=dtype
                )
                ...
                if ik_success_grasp and ik_qpos_7d_grasp is not None:
                    print(f"为抓取位姿找到 IK 解: {ik_qpos_7d_grasp}")
                    print("构造 8D 动作并模拟移动到抓取位姿...")
                    
                    # 获取右臂当前 qpos 以保持其姿态
                    agent_right = env.agent.agents[1]
                    qpos_right_current_grasp = agent_right.robot.get_qpos() # Shape [1, 13]
                    # 假设：控制 8 个关节，第 8 个是夹爪
                    gripper_joint_index = 7
                    num_controlled_joints = 8
                    action_grasp = None
                    ...
                    try:
                        ...
                        if qpos_left_current_grasp.shape[1] <= gripper_joint_index:
                             raise IndexError(f"左臂 qpos 维度不足以获取夹爪值 (索引 {gripper_joint_index})")
                        ...
                        # --- 构造右臂 8D 动作 (保持当前) ---
                        if qpos_right_current_grasp.shape[1] < num_controlled_joints:
                             raise IndexError(f"右臂 qpos 维度不足以获取前 {num_controlled_joints} 个受控关节")
                        ...
                        # --- 构建最终 Action 字典 --- 
                        action_grasp = {
                            left_agent_key: action_left_8d_grasp,
                            right_agent_key: action_right_8d_grasp
                        }
                        print(f"构建抓取动作为: {left_agent_key} shape={action_grasp[left_agent_key].shape}, {right_agent_key} shape={action_grasp[right_agent_key].shape}")
                    
                    except IndexError as idx_err:
                        print(f"错误: 构造抓取 8D 动作时发生索引错误: {idx_err}")
                    except Exception as dict_build_err:
                         print(f"错误: 构建抓取动作字典时出错: {dict_build_err}")
                    
                    # --- 循环调用 env.step(action_grasp) ---
                    if action_grasp is not None:
                        num_steps_grasp = 50 # 为抓取移动设定的步数
                        print(f"开始执行 {num_steps_grasp} 步抓取模拟...")
                        for step_idx in range(num_steps_grasp):
                            try:
                                obs, reward, terminated, truncated, info = env.step(action_grasp)
                                if env.render_mode == "human": env.render()
                            except Exception as step_err:
                                 print(f"抓取 env.step() 在第 {step_idx+1} 步出错: {step_err}")
                                 import traceback; traceback.print_exc()
                                 break # 出错则停止
                        ...
                        print("错误: 抓取动作构建失败，跳过 env.step() 循环。")
                        
                else: # IK 失败
                     print("为抓取位姿进行的 IK 失败或未找到解，跳过移动步骤。")
                # --- 抓取移动结束 ---
                
            else:
                 print("\n未能获取有效的 GraspNet 抓取位姿 (sapien.Pose)，跳过后续 IK 和移动步骤。")
            # --- 基于 GraspNet 结果进行 IK 和移动 结束 ---
